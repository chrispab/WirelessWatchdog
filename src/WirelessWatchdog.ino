...
#define TX433PIN 5
#define RX433PIN 2
#define RF24_CEPIN 7			 // The pin attached to Chip Enable on the RF module
#define RF24_CSPIN 8			 // The pin attached to Chip Select
#define SWITCH_PIN A7			 //the number of the pushbutton pin
#define ANALOGUE_SWITCHES_PIN A0 // ip port A0
#define REDLED_PIN 3			 //;
#define GREENLED_PIN 10			 //;
#define BLUELED_PIN 9			 //;
...
LedFader heartBeat(GREENLED_PIN, 0, 20, 900);
LedFader rebootAlert(REDLED_PIN, 0, 20, 456);
LedFader blueAlert(BLUELED_PIN, 0, 0, 1321);
...
void loop(void)
{
	...
	x = analogRead(ANALOGUE_SWITCHES_PIN);
	//Serial.println((x));
	displayKeys(x);
	//Serial.println(x);
	//u8x8.clear();
	...
	//pinMode(SWITCH_PIN, INPUT);
	switchState = analogRead(SWITCH_PIN);
	//switchState = 1;
